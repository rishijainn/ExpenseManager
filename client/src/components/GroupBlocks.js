...
function GroupBlocks({ grpName, grpId, setViewGroup ,refresh,setRefresh}) {
  ...
  const view=(grp)=>{
    axios.get(`https://expense-manager-backend-eight.vercel.app/user/data/getGroupData/${grp}`,{withCredentials:true})
    .then((response)=>{
  ...
  const leaveGroup = () => {
    ...
    axios.post(`https://expense-manager-backend-eight.vercel.app/user/data/leaveGroup`,{ groupId:grpId,userId,userName }, {withCredentials: true})
    .then((response) => {
        console.log(response.data.response);
